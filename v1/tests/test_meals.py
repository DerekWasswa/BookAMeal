...
class MealTests(BaseCaseTest):
	...
	def test_adding_a_meal_option(self):
		...
		response = self.client.post('/api/v1/meals/', data = self.all_food_meal, headers = self.headers_with_token(login_response['token']))
	...
	def test_adding_existing_meal_option(self):
		...
		#Testing addition of a meal option should add
		response_first = self.client.post('/api/v1/meals/', data = self.all_food_meal, headers = self.headers_with_token(login_response['token']))

		response_existing = self.client.post('/api/v1/meals/', data = self.all_food_meal, headers = self.headers_with_token(login_response['token']))
	...
	def test_modifying_a_meal_option(self):
		...
		response = self.client.post('/api/v1/meals/', data = self.matooke_meal, headers = self.headers_with_token(login_response['token']))
	...
	def test_getting_all_meals(self):
		...
		response_add = self.client.post('/api/v1/meals/', data = self.all_food_meal, headers = self.headers_with_token(login_response['token']))
	...
	def test_adding_meal_price_that_is_not_an_int(self):
		...
		response = self.client.post('/api/v1/meals/', data = self.stringed_meal_price, headers = self.headers_with_token(login_response['token']))
		result = json.loads(response.data.decode())
		self.assertEqual(response.status_code, 400)
	...
	def test_adding_a_meal_with_empty_request_parameters(self):
		...
		response = self.client.post('/api/v1/meals/', data = self.meal_with_wrong_request, headers = self.headers_with_token(login_response['token']))
		result = json.loads(response.data.decode())
		self.assertEqual(response.status_code, 400)
	...
	def test_meal_update_not_found(self):
		...
		self.assertIn('Update Incomplete! Meal does not exist.', str(response_edit_meal.data))
	...
	def test_add_meal_with_unauthorized_access(self):
		...
		response = self.client.post('/api/v1/meals/', data = self.all_food_meal, headers = self.headers_with_token(unpriviledged_response['token']))
	...
	def test_retrieving_meals_with_unauthorized_access(self):
		...
		response_get = self.client.get('/api/v1/meals/', headers = self.headers_with_token(unpriviledged_response['token']))
	...
	def test_deleting_a_meal_with_empty_meals(self):
		...
		response_delete_meal = self.client.delete('/api/v1/meals/{}' . format(15), headers = self.headers_with_token(login_response['token']))
	...
	def test_deleting_meal_not_found(self):
		...
		# Test that the API does not allow for deletion of a meal option which does not exist
		response_delete_meal = self.client.delete('/api/v1/meals/{}' . format(20), headers = self.headers_with_token(login_response['token']))
		self.assertEqual(response_delete_meal.status_code, 404)
		self.assertIn("Deletion Incomplete! Meal Not Found.", str(response_delete_meal.data))
	...
	def test_add_meal_with_no_auth_token(self):
		...
		response = self.client.post('/api/v1/meals/', data = self.all_food_meal, headers = self.no_token_headers)
		self.assertEqual(response.status_code, 401)
	...
	def test_add_meal_with_invalid_token(self):
		...
		response = self.client.post('/api/v1/meals/', data = self.all_food_meal, headers = self.invalid_token_headers)
		self.assertEqual(response.status_code, 401)
