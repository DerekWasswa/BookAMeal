...
class MenuTests(BaseCaseTest):
	...
	def test_set_menu_of_the_day(self):
		...
		response_one = self.client.post('/api/v1/meals/', data = self.all_food_meal, headers = self.headers_with_token(login_response['token']))

		response = self.client.post('/api/v1/menu/', data = self.caterer_menu, headers = self.headers_with_token(login_response['token']))
	...
	def test_setting_empty_menu(self):
		...
		response = self.client.post('/api/v1/menu/', data = self.empty_menu, headers = self.headers_with_token(login_response['token']))
	...
	def test_setting_menu_of_the_day_with_empty_request_parameters(self):
		...
		response = self.client.post('/api/v1/menu/', data = self.caterer_menu_wrong_request_params, headers = self.headers_with_token(login_response['token']))
		result = json.loads(response.data.decode())
		self.assertEqual(response.status_code, 400)
		self.assertEqual(result['message'], 'Setting a Menu expects Menu name, date, description, and meal Id keys.')
	...
	def test_setting_menu_with_unauthorized_access(self):
		...
		response = self.client.post('/api/v1/menu/', data = self.caterer_menu, headers = self.headers_with_token(unpriviledged_response['token']))
	...
	def test_setting_menu_of_the_day_meal_option_does_not_exist(self):
		...
		self.assertIn("Meal with provided ID does not exist.", str(response.data))
