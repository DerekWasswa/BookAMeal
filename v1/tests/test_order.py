...
class OrderTests(BaseCaseTest):
    ...
    def test_modifying_an_order(self):
        ...
        results_get_order_by_id = self.client.get(
            ...
            headers=self.headers_with_token(
                login_response['token']))
        self.assertIn(str(posted_data['order']['order_id']), str(
    ...
    def test_making_order_with_valid_email_and_order_id_of_type_int(self):
        # Testing making an order from the menu does not allow users with
        # invalid emails
        self.mock_signup()
        self.mock_login()
    ...
    def test_making_orders_with_empty_request_parameters(self):
        # Making an order with Empty Request parameters should not go ahead to
        # execute
        self.mock_signup()
        self.mock_login()
    ...
    def test_order_modification_with_empty_request_parameters(self):
        # Order modification with Empty Request parameters should not go ahead
        # to execute
        self.mock_signup()
        login = self.mock_login()
        login_response = json.loads(login.get_data(as_text=True))
    ...
    def test_getting_all_orders(self):
        ...
        response_get_orders = self.client.get(
            '/api/v1/orders/',
            headers=self.headers_with_token(
                login_response['token']))
